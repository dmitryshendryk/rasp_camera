...
class MQTTClient():
    ...
    def __init__(self, camera_obj, rpi_config=None):
        ...
        self.mqttc.message_callback_add("store/prishna/rpi/actions/upload_videos", self.upload_video_to_s3)
    ...
    def clear_videos(self,mqttc, obj, msg):
        ...
        if self.rpi_id == msg['rpi_id']['rpis'] and self.local_config['type'] == msg['type'] and self.local_config['location'] == msg['rpi_id']['region']: 
            ...
            now = datetime.now()
            date_time = now.strftime("%m/%d/%Y, %H:%M:%S")
            blob = json.dumps({'time': str(now), 'region':self.local_config['location'], 'node': self.rpi_id, 'node_type': self.local_config['type'], 'log': 'Clear Videos'})
            self.publish_message('/logs/rpi/' + self.local_config['type'] + '/', blob)
